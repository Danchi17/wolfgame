            ...
            case '占い師':
                actionArea.innerHTML = `
                    ...
                    ${gameState.players.map(player => 
                        ...
                        `<button onclick="window.performAction('seer', '${player.id}')">占う: ${player.name}</button>` : 
                        ''
                    ).join('')}
                    <button onclick="window.performAction('seer', 'graveyard')">墓地を占う</button>
                `;
                break;
            case '怪盗':
                actionArea.innerHTML = `
                    ...
                    ${gameState.players.map(player => 
                        player.id !== currentPlayer.id ? 
                        `<button onclick="window.performAction('thief', '${player.id}')">交換: ${player.name}</button>` : 
                        ''
                    ).join('')}
                    <button onclick="window.performAction('thief', null)">交換しない</button>
                `;
                break;
            case '人狼':
                ...
                actionArea.innerHTML = otherWerewolf ? 
                    ...
                    '<p>あなたは唯一の人狼です。</p>';
                actionArea.innerHTML += `<button onclick="window.performAction('werewolf', null)">確認</button>`;
                break;
        }
